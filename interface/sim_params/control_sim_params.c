...
/* IV) Open the data file and read in the information                    */
 ...
 while(flag_test){
   if(myid==0){  
     printf("after  fp open %d %s %d ttt\n",ind_key,fun_key,nfun_key);
    fflush(stdout);
   }
   ...
   if(myid==0){  
     printf("after  fp open %d %s %d sss \n",ind_key,fun_key,nfun_key);
     printf("after  fp open %s \n",input_name);
    fflush(stdout);
   }
    nkey  = 0;
    while(get_word(fp,&word,&nline,&nkey,nfun_key,input_name)){
   if(myid==0){  
     printf("after  fp open 111 %d \n",ind_key);
    fflush(stdout);
   }
      switch(ind_key){
        ...
        case 7 : put_word_dict(&word,dict_vol,num_dict_vol,fun_key,nline,
                               nkey,nfun_key,input_name);break;
      case 8 :   if(myid==0){  
	  printf("after  fp open 001 %s\n",word.keyword);
	  fflush(stdout);
	}
	put_word_dict(&word,dict_write,num_dict_write,fun_key,nline,
		      nkey,nfun_key,input_name);
	if(myid==0){  
	  printf("after  fp open 002 \n");
	  fflush(stdout);
	}
	break;
   ...
   if(myid==0){  
     printf("after  fp open 222 %d \n",ind_key);
    fflush(stdout);
   }
    }/* end while */
   if(myid==0){  
     printf("after  fp open 333 %d \n",ind_key);
    fflush(stdout);
   }
  flag_test=get_fun_key(fp,fun_key,&nline,&nfun_key,input_name);
   if(myid==0){  
     printf("after  fp open 444 %d %d\n",ind_key,flag_test);
    fflush(stdout);
   }
  }/*end while*/
   if(myid==0){  
  printf("after reading in 3 \n");
    fflush(stdout);
   }
